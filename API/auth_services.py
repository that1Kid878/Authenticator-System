...
def Hash_Refresh_Token(Token:str):
    New_Token = hmac.new(
        REFRESH_TOKEN_PEPPER.encode(),
        Token.encode(),
        hashlib.sha256
    ).digest()
    return New_Token.hex()

def Rotate_Refresh_Token(DB_Token:Refresh_Token, Token:str, New_Token_Id:uuid.UUID, DB:db_dependency):
...
def Check_Refresh_Token(Token:str, DB:db_dependency):
    All_Valid_Refresh_Tokens: list[Refresh_Token] = DB.query(Refresh_Token).filter(Refresh_Token.expires_at > datetime.now(timezone.utc)).all()
    Hashed_Token = Hash_Refresh_Token(Token)
